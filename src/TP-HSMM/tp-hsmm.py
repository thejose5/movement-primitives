...
class TP_HSMM:
    def __init__(self, data_addr, num_viapts, ndemos, dt=0.1):
        ...
        # self.global_gmm.predict([900,380])
        ######################## TEST #########################
    ...
    def encodeDuration_and_stateTransition(self):
        prev_state = -1
        transition_matrix = np.empty((self.num_gmm_components,self.num_gmm_components))
        duration_matrix = np.empty((self.num_gmm_components, self.num_demos))
